 ...
 */
export const searchImagerySchema = z.object({
  page: z.string().optional().describe("Pagination cursor returned by a previous search_imagery call"),
  aoi: z.string().optional().describe("Area of interest as WKT POLYGON string"),
  fromDate: z.string().optional().describe("Start date (ISO 8601, e.g. 2024-01-01)"),
  toDate: z.string().optional().describe("End date (ISO 8601, e.g. 2024-06-01)"),
  ...
  if (!params.page && (!params.aoi || !params.fromDate || !params.toDate)) {
    ctx.addIssue({
      ...
      message: "Initial searches require aoi, fromDate, and toDate unless page is provided.",
    });
  }
});
