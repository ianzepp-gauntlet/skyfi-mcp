...
This keeps route decomposition inspectable before feasibility runs and uses the same batch feasibility path for ordinary polygons and corridor chunks. The `feasibility_status` response uses snake_case fields throughout, including aggregate counters such as `request_count` and nested per-request fields such as `feasibility_id`.
